...
test("vendor add는 파일과 정확한 commit을 함께 기록한다", async () => {
  ...
  expect(await readFile(join(target, "skills/demo/reference.md"), "utf8")).toBe("base\n");
...
test("vendor update는 로컬 수정과 upstream 수정을 3-way merge한다", async () => {
  ...
  const localSkill = join(target, "skills/demo/SKILL.md");
  ...
  expect(await readFile(join(target, "skills/demo/reference.md"), "utf8")).toBe("upstream v2\n");
