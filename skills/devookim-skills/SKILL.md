...
Use `bunx devookim-skills help` as the command source of truth.

## Install skills

1. Run `bunx devookim-skills find` when the skill name is unknown.
2. Use `add`, `update`, or `remove` for installed copies.
...
2. Use `vendor add` to copy the complete skill directory into `skills/<name>/` and record its exact upstream commit in `skills-lock.json`.
...
Treat `skills/<name>/` and its `skills-lock.json` entry as one change. Preserve local modifications during upstream updates and inspect the resulting diff before reporting completion.
